...
check_aws_cli() {
    ...
    if [[ "${current_version}" < "${min_version}" ]]; then
        error "AWS CLI version ${current_version} is less than required version ${min_version}"
        exit 1
    fi
}
...
validate_config() {
    ...
    if ! [[ "${RETRY_ATTEMPTS}" =~ ^[0-9]+$ ]] || \
       ...
       ! [[ "${PARALLEL_JOBS}" =~ ^[0-9]+$ ]]; then
        error "Invalid numeric configuration values"
        exit 1
    fi
}
...
process_account() {
    log "Processing AWS account: $(aws sts get-caller-identity --query 'Account' --output text)"
...
process_ami() {
    ...
    # Check encryption
    check_ami_encryption "${ami_id}"
    
    # Check age
    check_ami_age "${ami_id}"
    
    # Check public access
    check_ami_public_access "${ami_id}"
}
...
check_ami_encryption() {
    ...
    encryption_status=$(aws ec2 describe-images \
        --image-ids "${ami_id}" \
...
check_ami_age() {
    ...
    if (( age_days > THRESHOLDS["ami_age_days"] )); then
        error "AMI ${ami_id} is ${age_days} days old (threshold: ${THRESHOLDS["ami_age_days"]} days)"
    fi
}
...
check_ami_public_access() {
    ...
    if [[ "${public_access}" == "true" ]]; then
        error "AMI ${ami_id} is publicly accessible"
    fi
}
...
process_csv() {
    ...
    done < <(tail -n +2 "$1") # Skip header row
}
