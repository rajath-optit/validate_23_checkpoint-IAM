...
print("\033[94m 1.Applications should expose to public via port 443 only.\033[0m")
print(" This script will check all security groups in your AWS account and see if any of them allow inbound traffic on port 443 from any IP address. If it finds such a security group, it will return a message confirming that port 443 is allowed. Otherwise, it will return a message indicating that port 443 is not allowed.")
...
def check_shield_enabled():
    ...
    if response['Subscription']['SubscriptionType'] == 'SHIELD_ADVANCED':
        return True
    else:
        return False
...
def main():
    while True:
        ...
        if not cloudfront_associated and not alb_associated:
            print("\033[91m Cannot scan this checkpoint as neither CloudFront distributions nor Application Load Balancers are associated with the application.\033[0m")
            continue
        ...
        if waf_enabled and shield_enabled:
            print("\n \033[92m Yes, all traffic from end users passes through AWS WAF and AWS Shield.\033[0m")
        else:
...
print("\033[94m 4.Applications should be enabled with Horizontal load balancers (Auto scaling) to meet the surge in traffic.\033[0m")
print("This script checks if there are any auto scaling groups associated with your application. If auto scaling groups are found, it indicates that your application is enabled with horizontal load balancers (auto scaling) to meet the surge in traffic. Otherwise, it indicates that this requirement is not met.")
...
def main():
    ...
    if auto_scaling_enabled:
        print("\033[92m Yes, applications are enabled with Horizontal load balancers (Auto scaling) to meet the surge in traffic.")
    else:
...
def main():
    ...
    if security_enabled:
        print("\033[92m Yes, application servers have some level of security measures such as IPS/IDS and DDoS protection.\033[0m")
    else:
...
print("This script checks all RDS DB instances in your AWS account and verifies if any of them have one or more read replicas. If read replicas are found, it indicates that a Master - Slave Architecture is set up for the database. Otherwise, it indicates that the requirement is not met.")
...
def main():
    ...
    if managed_db_exists:
        print("\033[92m A managed database such as Amazon RDS, Amazon Aurora, or another option is associated with this application.\033[0m")
    else:
...
print("This script describes all EBS volumes in the AWS account and checks if they are encrypted. If any volume is found to be unencrypted, it adds the volume ID to the list of unencrypted volumes. Finally, it prints whether all EBS volumes are encrypted or lists the unencrypted volumes if any are found.")
...
def main():
    ...
    if unencrypted_volumes:
        print("\033[91m Not all EBS volumes are encrypted.")
        print("\033[91m List of unencrypted EBS volumes:")
...
print("This script lists all S3 buckets in the AWS account and checks if they are encrypted. If any bucket is found to be unencrypted or does not have encryption settings configured, it adds the bucket name to the list of unencrypted buckets. Finally, it prints whether all S3 buckets are encrypted or lists the unencrypted buckets if any are found.")
...
def main():
    ...
    if unencrypted_buckets:
        print("\033[91mNot all S3 buckets are encrypted.\033[0m")
        print("\033[91mList of unencrypted S3 buckets:\033[0m")
...
def main():
    ...
    if unversioned_buckets:
        ...
        print("All S3 buckets have versioning enabled.\033[0m")
...
print("------------------------------------------------------------------------------")
print("This script prompts the user to enter the IP address of the Bastion Host in CIDR notation.")
...
print("Otherwise, it prints 'SSH access to production resources is NOT limited to the Bastion Host ONLY.'")
...
print("--------------------------------------------------------------------------------------")
print(" This script prompts the user to enter the name of the IAM user or role associated with the Bastion Host.")
print(" It then checks if MFA is required for SSH access by examining the IAM policies attached to that user or role.")
...
print(" It verifies if access to a Bastion Host is restricted to a VPN. It asks for Bastion Host security group IDs, optionally allows manual input, and prompts for the VPN's IP range. Then, it checks if SSH access is restricted to the VPN's IP range.")
...
print(" The script checks if Multi-Factor Authentication (MFA) is enabled for VPN access. It prompts for the IAM user or role associated with VPN access, then examines attached IAM policies and groups to determine MFA status. If MFA is enabled, it prints 'MFA enabled for VPN access'; otherwise, it prints 'MFA not enabled for VPN 'access'.")
